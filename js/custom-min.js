...
(function ($) {
	...
	/* VARIABLES                                           */
	/* ==================================================== */
	const navBar = $('.custom-menu');

	const navbarLinks = $('.custom-menu .nav-link');
	...
	const $ClientsSlider = $('.testimonial-slider');
	...
	const swiper = new Swiper('.screen-slider', {
		...
		iframe: {
			patterns: {
				...
				vimeo: {
					index: 'vimeo.com/',
					id(url) {
						const m = url.match(/(https?:\/\/)?(www.)?(player.)?vimeo.com\/([a-z]*\/)*(\d{6,11})[?]?.*/);
						if (!m || !m[5]) {
							return null;
						}
						return m[5];
